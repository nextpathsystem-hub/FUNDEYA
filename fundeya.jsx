...
const Btn = ({ children, onClick, style = {}, variant = "primary", loading = false }) => {
  const base = { width: "100%", border: "none", borderRadius: 12, padding: "13px", fontSize: 15, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", transition: "opacity 0.15s" };
  const variants = {
    ...
    ghost: { background: C.gray, color: C.text },
  };
  return <button onClick={onClick} style={{ ...base, ...variants[variant], opacity: loading ? 0.6 : 1, ...style }}>{loading ? "Chargement..." : children}</button>;
};

const Input = ({ label, ...props }) => (
  <div style={{ marginBottom: 14 }}>
    {label && <p style={{ margin: "0 0 6px", fontSize: 13, fontWeight: 500, color: C.textS }}>{label}</p>}
    <input style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: `1.5px solid ${C.grayM}`, fontSize: 14, color: C.text, background: C.gray, outline: "none" }} {...props} />
  </div>
);
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "40px 24px 24px" }}>
      ...
      <div style={{ display: "flex", background: C.gray, borderRadius: 12, padding: 4, marginBottom: 24 }}>
        {[["login", "Connexion"], ["signup", "Inscription"]].map(([m, l]) => (
          <button key={m} onClick={() => setMode(m)} style={{ flex: 1, padding: "9px", borderRadius: 10, border: "none", cursor: "pointer", fontWeight: 500, fontSize: 14, background: mode === m ? C.white : "none", color: mode === m ? C.navy : C.grayT, boxShadow: mode === m ? "0 1px 4px rgba(0,0,0,0.08)" : "none" }}>{l}</button>
        ))}
      </div>

      {mode === "signup" && <Input label="Ton prénom et nom" placeholder="Grace Kabondo" value={name} onChange={e => setName(e.target.value)} />}
      <Input label="Email universitaire" type="email" placeholder="grace@etu.univ-nantes.fr" value={email} onChange={e => setEmail(e.target.value)} />
      ...
      <Btn onClick={handle} loading={loading}>{mode === "login" ? "Se connecter" : "Créer mon compte"}</Btn>
    </div>
  );
};
...
const OnboardingScreen = ({ token, userId, name, onDone }) => {
  ...
  const steps = [
    <div key="s0">
      ...
      <div style={{ marginTop: 24 }}><Btn onClick={() => setStep(1)} variant={level && field ? "primary" : "ghost"}>Continuer</Btn></div>
    </div>,
    <div key="s1">
      ...
      <Btn onClick={() => setStep(2)} variant={goals.length > 0 ? "primary" : "ghost"}>Continuer</Btn>
    </div>,
    <div key="s2">
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <textarea value={bio} onChange={e => setBio(e.target.value)} placeholder="Ex: Dispo les matins, j'aime travailler en Pomodoro..." style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: `1.5px solid ${C.grayM}`, fontSize: 14, color: C.text, background: C.gray, outline: "none", resize: "none", height: 80, fontFamily: "inherit" }} />
      </div>
      <Btn onClick={finish} loading={loading}>Créer mon profil</Btn>
    </div>
  ];
...
const Accueil = ({ profile }) => {
  ...
  return (
    <div style={{ padding: "20px 16px", overflowY: "auto", flex: 1 }}>
      ...
      <div style={{ background: C.white, border: `1px solid ${C.grayM}`, borderRadius: 14, padding: "14px 16px", display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
        <div style={{ position: "relative", width: 48, height: 48, flexShrink: 0 }}>
          <svg width="48" height="48" viewBox="0 0 48 48">
            <circle cx="24" cy="24" r="20" fill="none" stroke={C.grayM} strokeWidth="4" />
            <circle cx="24" cy="24" r="20" fill="none" stroke={C.teal} strokeWidth="4" strokeDasharray={`${(done / 4) * 125.6} 125.6`} strokeLinecap="round" transform="rotate(-90 24 24)" />
          </svg>
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 600, color: C.text }}>{done}/4</div>
        </div>
        <div style={{ flex: 1 }}>
          <p style={{ margin: "0 0 3px", fontWeight: 500, color: C.text, fontSize: 15 }}>Objectifs du jour</p>
          <p style={{ margin: 0, fontSize: 12, color: C.grayT }}>Clique pour cocher tes tâches</p>
        </div>
      </div>
...
const Decouvrir = ({ token, userId }) => {
  ...
  const affinityScore = (p) => {
    let score = 70 + Math.floor(Math.random() * 25);
    return score;
  };

  const scored = profiles.map(p => ({ ...p, affinity: affinityScore(p) })).sort((a, b) => b.affinity - a.affinity);
  const top = scored[0];
  ...
  return (
    <div style={{ overflowY: "auto", flex: 1 }}>
      <div style={{ padding: "20px 16px 0", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 600, color: C.text }}>Fundeya</h2>
          <p style={{ margin: 0, fontSize: 12, color: C.grayT }}>Nantes Université</p>
        </div>
      </div>
      ...
      {loading ? (
        <div style={{ padding: "40px 16px", textAlign: "center", color: C.grayT }}>
          <i className="ti ti-loader-2" style={{ fontSize: 28, display: "block", marginBottom: 10 }} />
            ...
            <div style={{ background: C.navy, borderRadius: 16, padding: 16, marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <p style={{ margin: 0, fontSize: 10, color: "rgba(255,255,255,0.5)", letterSpacing: "0.05em" }}>AFFINITÉ</p>
                </div>
              </div>
              {top.goals?.length > 0 && <p style={{ margin: "0 0 14px", fontSize: 13, color: "rgba(255,255,255,0.7)" }}>{top.goals.join(" · ")}</p>}
              <button onClick={() => connect(top.id)} style={{ width: "100%", background: connections.includes(top.id) ? C.teal : "#fff", border: "none", borderRadius: 10, padding: "11px", fontSize: 14, fontWeight: 600, color: connections.includes(top.id) ? "#fff" : C.navy, cursor: "pointer" }}>
                {connections.includes(top.id) ? "✓ Connecté·e" : "Envoyer un message"}
              </button>
            </div>
          )}
          {rest.length > 0 && (
                ...
                <div key={u.id} style={{ background: C.white, border: `1px solid ${C.grayM}`, borderRadius: 14, padding: 14, marginBottom: 10 }}>
                  ...
                  {u.goals?.length > 0 && (
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
                      {u.goals.slice(0, 2).map((g, j) => <span key={j} style={{ background: C.tealL, color: C.teal, fontSize: 12, padding: "4px 10px", borderRadius: 20, fontWeight: 500 }}>{g}</span>)}
                    </div>
                  )}
                  <button onClick={() => connect(u.id)} style={{ width: "100%", background: connections.includes(u.id) ? C.teal : C.navy, border: "none", borderRadius: 10, padding: "10px", fontSize: 14, fontWeight: 500, color: "#fff", cursor: "pointer" }}>
                    {connections.includes(u.id) ? "✓ Connecté·e" : "Se connecter"}
                  </button>
                </div>
              ))}
            </>
...
// MESSAGES
const MessagesScreen = ({ token, userId, profile }) => {
  ...
  const send = async () => {
    ...
    try {
      await api("messages", { method: "POST", token, body: JSON.stringify(msg) });
    } catch (e) { console.error(e); }
  };
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto" }}>
      ...
      {loading ? (
        <div style={{ padding: "40px", textAlign: "center", color: C.grayT }}>Chargement...</div>
      ) : convs.length === 0 ? (
        <div style={{ padding: "40px 24px", textAlign: "center" }}>
          ...
          {convs.map(p => (
            <button key={p.id} onClick={() => openConv(p)} style={{ width: "100%", background: "none", border: "none", cursor: "pointer", padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, borderBottom: `1px solid ${C.grayM}`, textAlign: "left" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ margin: 0, fontSize: 13, color: C.grayT }}>{p.level || "Étudiant·e"} · Appuie pour discuter</p>
              </div>
...
const ProfilScreen = ({ token, userId, profile, onLogout }) => {
  ...
  const saveMood = async (m) => {
    ...
    try { await api(`profiles?id=eq.${userId}`, { method: "PATCH", token, body: JSON.stringify({ mood: m }) }); }
    catch (e) { console.error(e); }
    setSaving(false);
  };
    ...
    <div style={{ overflowY: "auto", flex: 1 }}>
      <div style={{ padding: "20px 16px", display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ margin: 0, fontSize: 12, color: C.grayT, display: "flex", alignItems: "center", gap: 4 }}>
            <i className="ti ti-map-pin" style={{ fontSize: 12 }} /> {profile?.university || "Nantes Université"}
          </p>
        </div>
      ...
      {profile?.goals?.length > 0 && (
        <div style={{ padding: "0 16px", marginBottom: 16 }}>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {profile.goals.map((g, i) => <span key={i} style={{ background: C.tealL, color: C.teal, fontSize: 12, padding: "5px 12px", borderRadius: 20, fontWeight: 500 }}>{g}</span>)}
          </div>
        </div>
      )}
...
export default function App() {
  ...
  const onAuth = async (tk, u, nameHint) => {
    ...
    try {
      ...
      if (ps?.length && ps[0].level) {
        setProfile(ps[0]);
        setScreen("app");
      } else {
        setProfile(ps?.[0] || { id: u.id, name: nameHint || u.email });
  ...
  const onOnboardingDone = async () => {
    try {
      const ps = await api(`profiles?id=eq.${user.id}`, { token });
      setProfile(ps?.[0]);
    } catch (e) { }
    setScreen("app");
  };

  const logout = () => { setToken(null); setUser(null); setProfile(null); setScreen("auth"); };

  const appScreens = {
    accueil: <Accueil profile={profile} />,
    decouvrir: <Decouvrir token={token} userId={user?.id} />,
    messages: <MessagesScreen token={token} userId={user?.id} profile={profile} />,
